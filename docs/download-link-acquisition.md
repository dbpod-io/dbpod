...
2. 缓存过期/缺失 → 若配置了 mirror 先试 `<mirror>/mysql.json`（无元数据则报错并
   视为不可用），再依次尝试仓库 CDN：
   `https://cdn.jsdelivr.net/gh/dbpod-io/dbpod@master/internal/metadata/data/mysql.json`
   → `https://raw.githubusercontent.com/dbpod-io/dbpod/master/internal/metadata/data/mysql.json`，
   成功后写入本地缓存
